...
function checkAFKs(minutes) {// Makes sure DJs chat every x minutes, we want as much participation as possible, not AFK DJs
    ...
    if (checkAFKThirdStrike.length > 0) {
        ...
        log("AFK " + afkNames[Math.round(Math.random() * (afkNames.length - 1))] + ": " + AFKlist + " you've been removed from the DJ wait list, " + afkInsults[Math.round(Math.random() * (afkInsults.length - 1))], log.visible);
    }
}
