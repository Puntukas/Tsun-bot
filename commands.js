...
var commands = [
    ...
    new Command("info", function() {
        log("[!info] Tsun-bot " + version + " was developed by @Puntukas. Type !help for available commands.", log.visible);
    }),


    new Command("reminder", function(author, args) {
        if(args.length < 2) {
            log("@" + author + " [!reminder] please use in the form of '!reminder something or !reminder off", log.visible);
        } else if (args[1] == "off") {
            meetupUrl = "";
    ...
    new Command("specialgreeting", function(author, args) {
        if(args.length < 2) {
            log("@" + author + " [!specialgreeting] please use in the form of '!specialgreeting We are professional chill'erz", log.visible);
            return;
        }
...
//log("Restarting the bot, Bye!...", log.visible);// really not needed, if you don't see it come up it's already fucked
    ...
    new Command("eta", function(author) {
        var DJmsgs = ["you're already the DJ, get your ears cleaned out!"];
    ...
    new Command("addiction", function(author) {
        if (author == "Puntukas") {
            log("Sorry @Puntukas, but you've got some serious issues that even I cant fix. Consult a real doctor.", log.visible);
        }
        else {
    ...
    new Command("sympathy", function() {
        log("@Tsun-bot has no time for your sympathy", log.visible);
    }, null, null, false),
    ...
    new Command("suck", function() {
        log("!suck @architektas's dick for mod", log.info);
    }, null, null, false),
